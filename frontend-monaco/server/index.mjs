...
function hasExecutableEntry(challenge) {
  const config = challenge.configuration ?? {};
  const files = Array.isArray(config.baseFiles) ? config.baseFiles : [];
  const paths = new Set(files.map((file) => file.path));
  const executable = /\.(ts|mts|cts|tsx|js|mjs|cjs|jsx|java)$/;
  if (config.entry && paths.has(config.entry) && executable.test(config.entry)) {
    return true;
  }
  if (paths.has('main.ts') || paths.has('index.ts')) {
    return true;
  }
  if (files.some((file) => executable.test(file.path))) {
    return true;
  }
  return false;
}
...
mínimo 3 caracteres; precio no negativo). Escriba su solución en el IDE y
verifique con los botones Compilar/Enviar del sandbox.
`,
        },
      ],
      hiddenTests: [
    ...
    metadata: { version: 1, softDeleted: false, notes: 'Seed Angular standalone + Reactive Forms: formulario para validar en el sandbox.' },
  },
  {
...
válido; cantidad mínima 1. Escriba su solución en el IDE y verifique con los
botones Compilar/Enviar del sandbox.
`,
        },
      ],
      hiddenTests: [
    ...
    metadata: { version: 1, softDeleted: false, notes: 'Seed Angular standalone + Template Forms: formulario para validar en el sandbox.' },
  },
  {
...
`,
  ...
  {
    challengeId: 'rf-carrito-refactor-single-medio',
    courseCohortId: 'TUP-2026-01',
    title: 'Refactorizar el cálculo del total (single-file)',
    topic: 'Refactoring — extraer funciones y constantes nombradas',
    subtype: 'refactoring',
    difficulty: 'MEDIO',
    mandatory: false,
    configuration: {
      ...
      baseFiles: [
        {
          path: 'main.ts',
          content: `// El código funciona pero tiene problemas de diseño. Refactorícelo sin cambiar
...
`,
      ...
      hiddenTests: [],
      expectedSolution: `const IVA = 0.21;
...
`,
    ...
    metadata: { version: 1, softDeleted: false, notes: 'Seed de demostración single-file (refactoring, riesgo MEDIO).' },
  },
  {
    challengeId: 'md-biblioteca-modelado-single-medio',
    courseCohortId: 'TUP-2026-01',
    title: 'Modelar el dominio de préstamos (single-file)',
    topic: 'Modelado de dominio — tipos, reglas y préstamo',
    subtype: 'modeling',
    difficulty: 'MEDIO',
    mandatory: false,
    configuration: {
      ...
      baseFiles: [
        {
          path: 'main.ts',
          content: `// Modelado de dominio: complete las reglas de préstamo de una biblioteca.
//
// Reglas de negocio:
...
const DIAS_DE_PRESTAMO = 14 as const;
...
function prestar(copia: Copia, usuario: string, prestamos: Prestamo[]): Prestamo | null {
  // TODO: aplique las reglas 1, 2 y 3.
  return null;
}
`,
        },
      ...
      hiddenTests: [],
      expectedSolution: `const DIAS_DE_PRESTAMO = 14;
const MAX_PRESTAMOS_VIGENTES = 3;
...
`,
    ...
    metadata: { version: 1, softDeleted: false, notes: 'Seed de demostración single-file (modeling, riesgo MEDIO).' },
  },
  {
    challengeId: 'hk-playlist-recomendador-single-bajo',
    courseCohortId: 'TUP-2026-01',
    title: 'Recomendador musical (single-file)',
    topic: 'Hackathon — lógica de recomendación por clima',
    subtype: 'hackathon',
    difficulty: 'BAJO',
    mandatory: false,
    configuration: {
      ...
      baseFiles: [
        {
          path: 'main.ts',
          content: `// Hackathon: construya el recomendador musical de la consigna.
...
`,
      ...
      hiddenTests: [],
      expectedSolution: `interface Cancion {
  titulo: string;
  genero: string;
  energia: number;
  duracionSeg: number;
...
`,
    ...
    metadata: { version: 1, softDeleted: false, notes: 'Seed de demostración single-file (hackathon, riesgo BAJO).' },
  },
];
...
async function ensureSeed() {
  ...
  const existingIds = new Set(challenges.map((c) => c.challengeId));
  const missing = SEED_CHALLENGES.filter((s) => !existingIds.has(s.challengeId));
  if (missing.length === 0) return;
  const now = new Date().toISOString();
  const stamped = missing.map((challenge) => ({
    ...challenge,
    metadata: {
      ...challenge.metadata,
      createdAt: now,
  ...
  await saveCollection('challenges', [...challenges, ...stamped]);
  console.log(`Seeded ${stamped.length} missing demo challenge(s).`);
}
