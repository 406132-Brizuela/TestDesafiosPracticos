...
describe('execution and verdicts (G05-E02)', () => {
  ...
  test('subtype sin runtime ni entrada ejecutable no es evaluable (422)', async () => {
    const created = await request('/api/challenges', {
      method: 'POST',
      body: {
        ...
        configuration: {
          language: 'markdown',
          entry: 'README.md',
          baseFiles: [{ path: 'README.md', content: '// material a revisar' }],
          hiddenTests: [],
        },
      },
    });
    ...
    const { status } = await request(`/api/practical-challenges/${id}/executions`, {
      method: 'POST',
      body: { files: [{ path: 'README.md', content: '// material' }], evaluate: true },
    });
    assert.equal(status, 422);
  });

  test('los seeds Angular sin runtime ahora son evaluables (200)', async () => {
    for (const id of ['app-producto-form-angular-rf', 'app-pedido-form-angular-tf']) {
      const { status } = await request(`/api/practical-challenges/${id}/executions`, {
        method: 'POST',
        body: { files: [{ path: 'src/app/product-form.component.ts', content: '// consigna' }], evaluate: true },
      });
      assert.equal(status, 200, `${id} should now be evaluable`);
    }
  });

  test('seeds single-file TS sin runtime se ejecutan en el sandbox', async () => {
    for (const id of [
      'rf-carrito-refactor-single-medio',
      'md-biblioteca-modelado-single-medio',
      'hk-playlist-recomendador-single-bajo',
    ]) {
      const challenge = await fullChallenge(id);
      const run = await request(`/api/practical-challenges/${id}/executions`, {
        method: 'POST',
        body: { files: challenge.configuration.baseFiles, entry: 'main.ts', evaluate: false },
      });
      assert.equal(run.status, 200, `${id} should be evaluable`);
      assert.equal(run.body.status, 'ok', `${id} should run in the sandbox`);
    }
  });
});
